...
export function AdjustWalletModal({ onConfirm, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative admin-card p-6 w-full max-w-sm animate-fade-in">
        <h3 className="text-lg font-semibold text-slate-100 mb-4">تنظیم کیف پول</h3>
        <div className="space-y-4 mb-4">
          <div>
            <label className="block text-sm text-slate-400 mb-2">مبلغ (تومان — منفی برای کم کردن)</label>
            <input
              type="number"
              value={amount}
              ...
              dir="ltr"
            />
            {amount !== 0 && <p className="text-xs text-slate-500 mt-1">{formatToman(Math.abs(amount))}</p>}
          </div>
          <div>
