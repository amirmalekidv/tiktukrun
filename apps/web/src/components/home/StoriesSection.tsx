...
export default function StoriesSection() {
  const { data: reviews = [], isLoading } = useSWR('home-reviews', () =>
    getAllReviews(HOME_REVIEWS_LIMIT)
  )
    ...
    <section className="py-8">
      <div className="section-header mb-6">
        <h2 className="font-cinzel font-black text-2xl text-white text-center">
          <span className="gradient-text">نظرات</span> پلیرها
        </h2>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="dark-card rounded-[20px] border border-white/10 p-6 text-center">
          ...
          <h3 className="mb-2 text-sm font-bold text-white">هنوز نظری ثبت نشده</h3>
