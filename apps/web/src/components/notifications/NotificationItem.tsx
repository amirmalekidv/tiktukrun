...
const TYPE_ICONS: Record<string, { icon: string; color: string }> = {
  booking: { icon: 'fa-calendar-check', color: '#00f5ff' },
...
export default function NotificationItem({ notification, index = 0 }: NotificationItemProps) {
  ...
  return (
    <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: index * 0.04 }}
      onClick={handleClick}
      className={`flex items-start gap-3 p-4 rounded-xl cursor-pointer transition-all border ${notification.isRead ? 'border-transparent hover:bg-white/[0.03]' : 'border-[#00f5ff]/20 bg-[#00f5ff]/10 hover:bg-[#00f5ff]/15'}`}>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between gap-2">
          ...
          {!notification.isRead && <div className="w-2 h-2 rounded-full bg-[#00f5ff] flex-shrink-0" />}
        </div>
